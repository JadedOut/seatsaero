...
class HybridScraper:
    ...
    def __init__(self, cookie_farm: CookieFarm, refresh_interval: int = 2,
                 ...
                 http_version: str = "h2", proxy: str = None):
        """
        Args:
            ...
            proxy: Proxy URL (e.g. socks5://user:pass@host:port). If not set,
                inherits from cookie_farm.proxy.
        """
        ...
        self._proxy = proxy or cookie_farm.proxy  # inherit from farm if not specified
    ...
    def start(self):
        ...
        session_kwargs = dict(impersonate="chrome142", http_version=self._http_version)
        if self._proxy:
            session_kwargs["proxies"] = {"https": self._proxy, "http": self._proxy}
...
# load_routes imported from core.routes
